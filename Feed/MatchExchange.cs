...
public class MatchExchange
{
    ...
    {
        if ((DateTime.Now - LastUpdateTime).TotalMilliseconds <= _checkPeriod)
            return;
    ...
    public bool IsEqual(MatchExchangesParameters matchExchange)
    {
        return matchExchange.Symbols == Symbol &&
               matchExchange.FirstExchange == Exchange &&
               matchExchange.SecondExchange == ExchangeForMatch.Exchange;
    }
}
